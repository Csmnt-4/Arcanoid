#pragma once
//verion 1.01 - fixed initial random distribution to a full circle, instead of 90 degrees
#include "Blit3D.h"
extern Blit3D *blit3D;

#include <random>
std::uniform_real_distribution<float> angle_dist(0, M_PI * 2); //360 degrees
...
private:
	...
	int panX = 960, panY = 540, panZ = 0; //use these to do whole pixel panning
...
public:
	...
	Camera2D()
	{
		...
		panOffset = glm::vec3(blit3D->screenWidth / 2, blit3D->screenHeight / 2, 0);
		minX = minY = minZ = 0;
		maxX = maxY = maxZ = 0;
		moveDir = glm::vec3(blit3D->screenWidth / 2, blit3D->screenHeight / 2, 0);
		panSpeed = 1000;
